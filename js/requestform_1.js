            ...
            var refreshVisibility45111 = function(){
                        var observers = [];
            ...
            //window.adsArea45111= el;
            window.adsArea45111.isVisible = null;
            window.adsArea45111.refreshTimer = Date.now();
            window.adsArea45111.formatId = adsAreaId;
            window.adsArea45111.refreshRate = refreshTimeTableVisible[adsAreaId];
            window.adsArea45111.capping = 1;
            window.adsArea45111.cappingLimit = adsCappingLimit;
            ...
            if ((Math.floor(entry.intersectionRatio * 100) > 50 && !window.adsArea45111.isVisible) ||
            window.adsArea45111.isVisible === null) {
            window.adsArea45111.isVisible = true;
            window.adsArea45111.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if ((Math.floor(entry.intersectionRatio * 100) < 50 && window.adsArea45111.isVisible) ||
            window.adsArea45111.isVisible === null) {
            window.adsArea45111.isVisible = false;
            window.adsArea45111.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            });
            };

            observers45111 = new IntersectionObserver(callback, option);
            observers45111.observe(document.querySelector("#sas_" + adsAreaId));

            } else {
            window.adsArea45111.lastPos = 0;
            window.adsArea45111.lastTime = Date.now();
            evt.scroll = window.addEventListener('scroll', function (e) {
            if ((Math.abs(window.adsArea45111.lastPos - window.scrollY) > 200) || (Date.now() - window.adsArea45111.lastTime > 500)) {
            window.adsArea45111.lastPos = window.scrollY;
            window.adsArea45111.lastTime = Date.now();
            ...
            if ((visibilitySensor && !window.adsArea45111.isVisible) ||
            window.adsArea45111.isVisible === null) {
            window.adsArea45111.isVisible = true;
            window.adsArea45111.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if ((!visibilitySensor && window.adsArea45111.isVisible) ||
            window.adsArea45111.isVisible === null) {
            window.adsArea45111.isVisible = false;
            window.adsArea45111.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            ...
            refreshQueueManager45111 = function (formatId, capping) {
            window.adsArea45111.capping++;
            window.adsArea45111.refreshTimer = Date.now();
            if(formatId == 26328){
            refreshSlotFooter(45111, window.adsArea45111.capping);
            }else{
            refreshSlot(45111, window.adsArea45111.capping);
            }
            }
            setInterval(function(){loopChecker45111();}, 1000);
            loopChecker45111 = function() {
            el = window.adsArea45111;
            if (!isNaN(el.refreshRate) && el.capping < el.cappingLimit && el.formatId != '26328') {
            lastBidder45111 = {"bidderCode":undefined, "responseTimestamp":0};
            tmzr.getAllWinningBids().forEach(function(bid){
            if(bid.adUnitCode==el.formatId){
            if(bid.responseTimestamp > lastBidder45111.responseTimestamp){
            lastBidder45111 = bid;
            }
            }
            })
            window.adsArea45111.lastBidder = lastBidder45111;

            if ((window.adsArea45111.refreshTimer + el.refreshRate) < Date.now() && el.lastBidder.bidder !== slowBidder) {
            refreshQueueManager45111(el.formatId, el.capping);
            ...
            if (window.adsArea45111.refreshTimer + 25000 < Date.now() && el.lastBidder.bidder !== slowBidder) {
            refreshQueueManager45111(el.formatId, el.capping);
            } else if ((window.adsArea45111.refreshTimer + slowdown < Date.now() && el.lastBidder.bidder == slowBidder)) {
            refreshQueueManager45111(el.formatId, el.capping);
            }
            ...
            if (window.adsArea45111.refreshTimer + el.refreshRate < Date.now()) {
            refreshQueueManager45111(el.formatId, el.capping);
            }
            }
            };
