            ...
            var refreshVisibility26326 = function(){
                        var observers = [];
            ...
            //window.adsArea26326= el;
            window.adsArea26326.isVisible = null;
            window.adsArea26326.refreshTimer = Date.now();
            window.adsArea26326.formatId = adsAreaId;
            window.adsArea26326.refreshRate = refreshTimeTableVisible[adsAreaId];
            window.adsArea26326.capping = 1;
            window.adsArea26326.cappingLimit = adsCappingLimit;
            ...
            if ((Math.floor(entry.intersectionRatio * 100) > 50 && !window.adsArea26326.isVisible) ||
            window.adsArea26326.isVisible === null) {
            window.adsArea26326.isVisible = true;
            window.adsArea26326.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if ((Math.floor(entry.intersectionRatio * 100) < 50 && window.adsArea26326.isVisible) ||
            window.adsArea26326.isVisible === null) {
            window.adsArea26326.isVisible = false;
            window.adsArea26326.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            });
            };

            observers26326 = new IntersectionObserver(callback, option);
            observers26326.observe(document.querySelector("#sas_" + adsAreaId));

            } else {
            window.adsArea26326.lastPos = 0;
            window.adsArea26326.lastTime = Date.now();
            evt.scroll = window.addEventListener('scroll', function (e) {
            if ((Math.abs(window.adsArea26326.lastPos - window.scrollY) > 200) || (Date.now() - window.adsArea26326.lastTime > 500)) {
            window.adsArea26326.lastPos = window.scrollY;
            window.adsArea26326.lastTime = Date.now();
            ...
            if ((visibilitySensor && !window.adsArea26326.isVisible) ||
            window.adsArea26326.isVisible === null) {
            window.adsArea26326.isVisible = true;
            window.adsArea26326.refreshRate = refreshTimeTableVisible[adsAreaId];
            } else if ((!visibilitySensor && window.adsArea26326.isVisible) ||
            window.adsArea26326.isVisible === null) {
            window.adsArea26326.isVisible = false;
            window.adsArea26326.refreshRate = refreshTimeTableInvisible[adsAreaId];
            }
            ...
            refreshQueueManager26326 = function (formatId, capping) {
            window.adsArea26326.capping++;
            window.adsArea26326.refreshTimer = Date.now();
            if(formatId == 26328){
            refreshSlotFooter(26326, window.adsArea26326.capping);
            }else{
            refreshSlot(26326, window.adsArea26326.capping);
            }
            }
            setInterval(function(){loopChecker26326();}, 1000);
            loopChecker26326 = function() {
            el = window.adsArea26326;
            if (!isNaN(el.refreshRate) && el.capping < el.cappingLimit && el.formatId != '26328') {
            lastBidder26326 = {"bidderCode":undefined, "responseTimestamp":0};
            tmzr.getAllWinningBids().forEach(function(bid){
            if(bid.adUnitCode==el.formatId){
            if(bid.responseTimestamp > lastBidder26326.responseTimestamp){
            lastBidder26326 = bid;
            }
            }
            })
            window.adsArea26326.lastBidder = lastBidder26326;

            if ((window.adsArea26326.refreshTimer + el.refreshRate) < Date.now() && el.lastBidder.bidder !== slowBidder) {
            refreshQueueManager26326(el.formatId, el.capping);
            ...
            if (window.adsArea26326.refreshTimer + 25000 < Date.now() && el.lastBidder.bidder !== slowBidder) {
            refreshQueueManager26326(el.formatId, el.capping);
            } else if ((window.adsArea26326.refreshTimer + slowdown < Date.now() && el.lastBidder.bidder == slowBidder)) {
            refreshQueueManager26326(el.formatId, el.capping);
            }
            ...
            if (window.adsArea26326.refreshTimer + el.refreshRate < Date.now()) {
            refreshQueueManager26326(el.formatId, el.capping);
            }
            }
            };
                ...
                var mydiv = document.getElementById("13364-16");
                ...
                sc.async = true;
                sc.text = sas.cmd.push(function(){sas.call('std',{siteId: 177885, pageId: 859707, formatId: 26326, target: crtg_content+moneybid});});
